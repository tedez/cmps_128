...
def parse_commit_link(http_str):
    ...
    if (matches) == 0:
        raise Exception(
            "Incorrect format for bitbucket commit string: " + hpttp_str)
    repo = matches[0][len('bitbucket.org/'):-len("/commits/")]
    matches = pattern_commit.findall(http_str)
    if (matches) == 0:
        raise Exception(
            "Incorrect format for bitbucket commit string: " + http_str)
...
def run(http_str, homeworknum):
    ...
    if status == 0:
        log(main_dir, "Successfully built container " +
            container_name + " for commit " + commit)
    else:
        log(main_dir, "Failed to build container " + container_name +
            "for commit " + commit + " status " + str(status))
    os.chdir(main_dir)
